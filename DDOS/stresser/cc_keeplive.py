...
while True:
    try:
        # 检查下轰炸进行还在不在，如果在的话，就继续进入循环，如果不在的话，就提交新的
        driver.find_element(By.CLASS_NAME,'attack-info')
        continue
    except:
        # 提交轰炸按钮
        ...
        for i in range(10):
            try:
                ...
                # 抓到验证码，并且截图
                driver.find_element(By.XPATH, '/html/body/div[3]/div/div/div[2]/div/div[1]/div/img').screenshot('cc_keeplive.png')
                time.sleep(1)
                # 用ddddocr来识别内容
                ocr = ddddocr.DdddOcr()
                with open('cc_keeplive.png', 'rb') as fp:
                    image = fp.read()
                # 拿到验证码catch
