    ...
    try:
        ...
        for i in range(10):
            try:
                ...
                # 抓到验证码，并且截图
                driver.find_element(By.XPATH,'/html/body/div[3]/div/div/div[2]/div/div[1]/div/img').screenshot('tangu_cc.png')
                time.sleep(1)
                # 用ddddocr来识别内容
                ocr = ddddocr.DdddOcr()
                with open('tangu_cc.png','rb') as fp:
                    image = fp.read()
                # 拿到验证码catch
